...
impl Target {
    pub(crate) fn new(
        ...
        Target {
            label: hotkey
                .map(|k| format!("敌人(被锁定)属性 ({k})"))
                .unwrap_or_else(|| "敌人(被锁定)属性".to_string()),
            alloc_addr,
            detour_addr,
...
impl Widget for Target {
    ...
    fn render_closed(&mut self, ui: &imgui::Ui) {
        ...
        pbar("生命条", hp, max_hp, COLOR_HP);
        pbar("集中力", sp, max_sp, COLOR_SP);
        pbar("耐力条", mp, max_mp, COLOR_MP);
        ...
        pbar("咒死", blight, blight_max, COLOR_BLIGHT);
